...
export default function HandVsRangePreflopTraining({ role, isAdmin }: { role: Role; isAdmin: boolean }) {
  ...
  useEffect(() => {
    getHandProgress(progressDifficulty, "preflop").then(async (p) => {
      setProgress(p);
  ...
  async function handleGuess(classIndex: number) {
    ...
    const refreshed = await getUnlockedDifficulties("preflop");
