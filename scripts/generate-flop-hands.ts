/**
 * Generates pre-computed flop hands with high-accuracy Monte Carlo simulation.
 * Run with: npx tsx scripts/generate-flop-hands.ts
 *
...
function generateFlop(exclude: { rank: string; suit: string }[]): { rank: string; suit: string }[] {
...
async function main() {
  console.log(`Generating ${TOTAL} flop hands with ${SIMS.toLocaleString()} simulations each...`);
  const startTime = Date.now();

  for (let i = 0; i < TOTAL; i++) {
    const handType = drawRandomHandType();
    const heroCards = generateHeroCards(handType);
    const flop = generateFlop(heroCards);
    const villainRange = VILLAIN_RANGES[Math.floor(Math.random() * VILLAIN_RANGES.length)];
    const equity = calculateEquity(heroCards, villainRange, flop, SIMS);
