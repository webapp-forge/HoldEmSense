/**
 * Generates pre-computed river hands with high-accuracy Monte Carlo simulation.
 * Run with: npx tsx scripts/generate-river-hands.ts
 *
...
function generateHeroCards(handType: string): { rank: string; suit: string }[] {
...
function drawCards(n: number, exclude: { rank: string; suit: string }[]): { rank: string; suit: string }[] {
...
async function main() {
  console.log(`Generating ${TOTAL} river hands with ${SIMS.toLocaleString()} simulations each...`);
  const startTime = Date.now();

  for (let i = 0; i < TOTAL; i++) {
    const handType = drawRandomHandType();
    const heroCards = generateHeroCards(handType);
    const flop = drawCards(3, heroCards);
    const [turnCard] = drawCards(1, [...heroCards, ...flop]);
    const [riverCard] = drawCards(1, [...heroCards, ...flop, turnCard]);
    const villainRange = VILLAIN_RANGES[Math.floor(Math.random() * VILLAIN_RANGES.length)];
    const equity = calculateEquity(heroCards, villainRange, [...flop, turnCard, riverCard], SIMS);
    ...
    await prisma.preGeneratedHand.create({
      data: {
        ...
        flopCard1Rank: flop[0].rank,
        flopCard1Suit: flop[0].suit,
        flopCard2Rank: flop[1].rank,
        flopCard2Suit: flop[1].suit,
        flopCard3Rank: flop[2].rank,
        flopCard3Suit: flop[2].suit,
